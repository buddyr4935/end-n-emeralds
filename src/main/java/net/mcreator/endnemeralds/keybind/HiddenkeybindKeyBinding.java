...
package net.mcreator.endnemeralds.keybind;
...
import net.mcreator.endnemeralds.procedures.HiddenkeybindOnKeyReleasedProcedure;
...
public class HiddenkeybindKeyBinding extends EndNEmeraldsModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;
	private long lastpress = 0;
	public HiddenkeybindKeyBinding(EndNEmeraldsModElements instance) {
		super(instance, 3);
	...
	public void initElements() {
		keys = new KeyBinding("key.end_n_emeralds.hiddenkeybind", GLFW.GLFW_KEY_H, "key.categories.misc");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					...
					EndNEmeraldsMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(1, dt));
		...
		if (type == 1) {
			{
				...
				HiddenkeybindOnKeyReleasedProcedure.executeProcedure($_dependencies);
			}
		}
	}
}
